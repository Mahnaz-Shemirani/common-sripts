...
USAGETXT=\
"
    Usage: $0 [options] <singularity container> <index dir> <fq file> <out dir> <--fldMean> <--fldSD>
...
# checks
[[ $# != 6 ]] && abort "This function expects 6 arguments"
...
[[ ! -d $4 ]] && abort "The forth argument needs to be an existing directory"



# create an output dir based on the sample name
outdir=$4/$(basename ${3/_1.f*q.gz/})
[[ ! -d $outdir ]] && mkdir -p $outdir

# run
singularity exec $1 salmon quant -p $CPU -i $2 -l $LTYPE -r $3 -o $outdir $OPTIONS 

# clean
[[ $CLEAN -eq 1 ]] && [[ -f $outdir/quant.sf ]] && rm $3 
